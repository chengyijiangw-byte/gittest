...
public class Server {
  ...
  public Server() {
    try {
            /*
                运行在服务端的ServerSocket可以理解为是电话的"总机"，可以同时接受多个
                客户端的链接。
                ServerSocket创建时要向系统申请服务端口，客户端就是通过这个端口与服务端
                建立链接的，并且ServerSocket要监听服务端口，这样一旦一个客户端链接ServerSocket
                会立即创建一个Socket与开客户端交互

                需要注意，实例化ServerSocket指定的服务端口不能与当前系统其他应用程序
                开启的服务端口冲突，否则会抛出异常:
  ...
  public void start() {
    try {
            /*
                ServerSocket提供的重要方法:
                Socket accept()
                该方法用于等待并接收客户端的链接，一旦一个客户端链接accept方法会立即
                返回一个Socket实例，通过这个Socket就可以与刚建立链接的客户端交互。
                如果没有客户端链接，那么accept方法会处于阻塞状态进行等待.
             */
      while (true) {
  ...
  private class ClientHandler implements Runnable {
    ...
    public void run() {
      try {
        ...
        String line;
                /*
                    当我们调用br.readLine()方法读取客户端发送过来一行字符串时,该方法会阻塞,
                    直到客户端发送过来一行字符串时,该方法会将这行内容返回.

                    当客户端断开链接时,服务端这里readLine方法会出现集中情况
                    1:客户端是异常断开的(客户端没有调用socket.close())
                      此时readLine方法会抛出异常:java.net.SocketException: Connection reset
                    2:客户端正常断开(客户端调用了socket.close())
                      此时readLine方法会返回null
                 */
